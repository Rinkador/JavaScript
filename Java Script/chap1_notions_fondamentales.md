...
<!-- public class CalculTTC {
    ...
    public static double calculerPrixTTC(double prixHT, double tauxTVA) {
        // Vérifier si le taux de TVA est valide (entre 0 et 1)
        if (tauxTVA < 0 || tauxTVA > 1) {
            throw new IllegalArgumentException("Le taux de TVA doit être compris entre 0 et 1.");
        }
    ...
    public static void main(String[] args) {
        ...
        double prixTTCDefaut = calculerPrixTTC(prixHT);
        System.out.println("Prix TTC avec taxe par défaut : " + prixTTCDefaut);

        // Utilisation de la fonction avec un taux de TVA personnalisé de 15%
        double tauxTVAPersonnalise = 0.15;
        double prixTTCPersonnalise = calculerPrixTTC(prixHT, tauxTVAPersonnalise);
        System.out.println("Prix TTC avec taxe personnalisée : " + prixTTCPersonnalise);
    }
}
 -->
...
<!-- function zip(arr1, arr2) {
  if (arr1.length !== arr2.length) {
    throw new Error("Les tableaux doivent avoir la même dimension");
  }
